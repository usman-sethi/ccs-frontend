...
import { Shield, Download, Eye, X, Award, CreditCard, CheckCircle2, Clock, ChevronDown, ChevronUp, QrCode } from "lucide-react";
...
import {
  getCertificatesFor, getTemplates, downloadCertificate,
  renderCertificateToCanvas, seedDemoTemplates, seedDemoIssuedCerts,
...
function CertCard({ cert, template, onView, onDownload, downloading }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-border bg-card">
      ...
      <div className="p-2">
        <p className="text-[10px] font-semibold truncate leading-tight">{cert.fieldValues?.title || cert.templateName}</p>
        <p className="text-[9px] text-muted-foreground">
          {cert.fieldValues?.date || new Date(cert.issuedAt).toLocaleDateString()}
        </p>
      </div>
      <div className="absolute inset-0 flex items-center justify-center gap-1.5 bg-black/55 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <Button size="sm" variant="secondary" className="h-7 px-2 text-xs gap-1"
          ...
          <Download className="size-3" /> {downloading === cert.id ? "…" : "Save"}
        </Button>
      </div>
    </div>
  );
...
function CertViewer({ cert, template, onClose, onDownload, downloading }) {
  ...
  useEffect(() => {
    ...
    renderCertificateToCanvas(canvasRef.current, template, cert.fieldValues, 900)
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl p-0 overflow-hidden">
        <DialogTitle className="sr-only">{cert.fieldValues?.title || cert.templateName}</DialogTitle>
        <DialogDescription className="sr-only">Certificate for {cert.recipientName}</DialogDescription>
        <div className="relative bg-muted min-h-40">
          <canvas
            ref={canvasRef}
            className="w-full"
            style={{ aspectRatio: `${template?.bgImageWidth ?? 1.414} / ${template?.bgImageHeight ?? 1}`, imageRendering: "pixelated" }}
          />
          {!rendered && (
        ...
        <div className="flex items-center justify-between border-t border-border px-4 py-3">
          <div>
            <p className="text-sm font-semibold">{cert.fieldValues?.title || cert.templateName}</p>
            <p className="text-xs text-muted-foreground">
              {cert.certNumber} · Issued {new Date(cert.issuedAt).toLocaleDateString()}
            </p>
          </div>
          <Button size="sm" disabled={downloading === cert.id} onClick={() => onDownload(cert)} className="gap-1.5">
            <Download className="size-3.5" /> {downloading === cert.id ? "Downloading…" : "Download"}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
function CardSection({ user }) {
  ...
  const handleApply = () => {
    const data = applyForCard(user?.displayName || "Member");
    setCardData(data);
  ...
  if (!cardData) return <div className="h-16 animate-pulse rounded-xl bg-muted" />;

  const name = user?.displayName || "Member";
  const dept = user?.department || "Computing";
  const since = user?.createdAt ? new Date(user.createdAt).getFullYear() : "2024";
        ...
        <div className="rounded-xl border border-dashed border-border bg-card/50 p-4 text-center space-y-3">
          ...
          <div className="mx-auto max-w-[260px] rounded-xl border-2 border-dashed border-border/50 bg-muted/30 aspect-[1.6] flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-[10px] text-muted-foreground/50 mt-1">Your card appears here</p>
            </div>
          </div>
          <p className="text-xs text-muted-foreground">Apply for your CCS member card</p>
...
export default function DashboardPage() {
  ...
  const loadCerts = useCallback(async () => {
    ...
    const name = user?.displayName || "Ayesha Khan";
    setCerts(getCertificatesFor(name));
  }, [user?.displayName]);

  useEffect(() => { loadCerts(); }, [loadCerts]);

  const findTemplate = (cert) => certTemplates.find((t) => t.id === cert.templateId);

  const handleDownloadCert = async (cert) => {
    const tmpl = findTemplate(cert);
    if (!tmpl) { toast.error("Certificate template not found."); return; }
  ...
  const onSubmit = async (values) => {
    setBusy(true);
    try {
      await updateProfile({ ...values, avatarUrl: avatar });
      ...
      <div className="border-b border-border bg-surface">
        <div className="container-page flex items-center justify-between py-10">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">{user?.email ?? "guest@ccs.dev"}</p>
          </div>
          {isAdmin && (
        ...
        <div className="grid gap-6 lg:grid-cols-[300px_1fr]">
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-col items-center gap-3 rounded-xl border border-border bg-card p-6">
              ...
              <div className="text-center">
                <p className="text-sm font-semibold">{user?.displayName || "Member"}</p>
                <p className="text-xs text-muted-foreground">{user?.email ?? "guest@ccs.dev"}</p>
                {isAdmin && (
              ...
              <Button size="sm" variant="outline" className="w-full" onClick={() => setAvatarDialog(true)}>
                Change photo
              </Button>
            </div>
                  ...
                  {certs.length > 4 && (
                    <Button variant="ghost" size="sm" className="w-full h-7 text-xs"
                      ...
                      {showAllCerts ? (
                        ...
                        <><ChevronDown className="size-3 mr-1" /> {certs.length - 4} more</>
                      )}
                    </Button>
                  )}
                </>
          ...
          >
            ...
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <Field label="Year" error={form.formState.errors.year?.message}>
                <Select value={form.watch("year")} onValueChange={(v) => form.setValue("year", v)}>
                  <SelectTrigger aria-label="Year"><SelectValue placeholder="Select" /></SelectTrigger>
                  <SelectContent>
            ...
            <Field label="Bio" error={form.formState.errors.bio?.message}>
              <Textarea rows={3} placeholder="A short intro…" {...form.register("bio")} />
