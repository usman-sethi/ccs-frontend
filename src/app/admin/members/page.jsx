...
export default function AdminMembersPage() {
  ...
  useEffect(() => {
    (async () => {
      const result = await backendMiddleware("/admin/members");
  ...
  const handleToggle = async (member) => {
    ...
    try {
      if (usingDemo) {
        ...
        toast.success(
          `${member.displayName} role updated (demo mode — not persisted).`,
        );
      } else {
        ...
        toast.success(
          `${member.displayName} is now ${!member.isAdmin ? "an admin" : "a member"}.`,
        );
      }
    } catch (e) {
    ...
    <div className="space-y-5">
      ...
      {usingDemo && (
        <div className="flex items-center gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-700 dark:text-amber-400">
          ...
          <span>
            Backend not connected — showing demo data. Set{" "}
